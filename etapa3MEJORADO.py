...
_opt=cargar_optica()
lam=_opt['lam']; alpha_l=_opt['alpha']; Phi_bin=_opt['Phi_bin']
R_frontal=0.10
Phi_tot=Phi_bin.sum()
Jsc_techo=q*Phi_tot*1e-4*1e3
G_optica=(1.0-R_frontal)*(np.exp(-np.outer(x,alpha_l))*(alpha_l*Phi_bin)).sum(axis=1)
Jsc_abs=q*_trapz(G_optica,x)*1e-4*1e3
...
voltajes_osc=np.arange(0.0,0.601,0.02)
G_cero=np.zeros(N)
psi,n,p=estado_inicial(); J_osc=[]
for V in voltajes_osc:
    psi,n,p,it=gummel(psi,n,p,V,G_cero)
...
psi,n,p,it=gummel(psi,n,p,0.0,G_optica,max_it=400)
Jsc=abs(corriente_terminal(psi,n,p))
Jx=perfil_J(psi,n,p)
unif=(np.max(np.abs(Jx))-np.min(np.abs(Jx)))/abs(np.mean(Jx))
...
V_light = np.arange(0.0, 0.661, 0.02)
psi,n,p = estado_inicial()
J_light_dir, its, unifs = [], [], []
for V in V_light:
    psi,n,p,it = gummel(psi,n,p,V,G_optica,max_it=400)
...
eta_dir = Pmax_dir/0.1*100

# --- Superposicion (metodo anterior), para comparar ---
Voc_sup = n_id*Vt*np.log(Jsc/J0+1.0)
Vg = np.linspace(0,Voc_sup,400)
Jl = Jsc - J0*(np.exp(Vg/(n_id*Vt))-1.0)
...
ax0b=ax0.twinx()
ax0.semilogy(x*1e6,G_optica,color='orange',lw=2)
ax0b.semilogy(lam,alpha_l/100.0,color='steelblue',lw=1.5)
ax0.axvline(W_emisor*1e6,color='gray',ls='--',lw=1)
