...
_opt=cargar_optica()
lam=_opt['lam']; al_lam=_opt['alpha']; Phi_bin=_opt['Phi_bin']
R_frontal=0.10
...
_psi0,_n0,_p0,_it0=gummel(_psi0,_n0,_p0,0.0,np.zeros(N))
...
Jsc_broad,it_b,unif_b=resolver_V0(G_broad)
print(f"  Jsc (banda ancha) = {Jsc_broad*1e3:.3f} mA/cm2  "
      f"(iters={it_b}, unif={unif_b:.1e})")

# ── 2. Barrido EQE(lambda) ─────────────────────────────────────────
print("\nBarrido EQE(lambda)...")
lam_eqe=np.linspace(300.,1100.,41)
Phi_mono=1e19                      # sonda lineal de baja intensidad
den_eqe=q*Phi_mono*1e-4
EQE=np.zeros_like(lam_eqe); iters=np.zeros_like(lam_eqe,dtype=int)
for k,lam0 in enumerate(lam_eqe):
    a_m=alpha_scaps_m(lam0)[0]
    G_mono=(1.0-R_frontal)*a_m*Phi_mono*np.exp(-a_m*x)
    Jk,itk=resolver_warm(G_mono)
    EQE[k]=Jk/den_eqe; iters[k]=itk
    if k%4==0 or lam0>1050:
        print(f"  lambda={lam0:6.0f} nm   EQE={EQE[k]*100:6.2f} %   (iters={itk})")
IQE=EQE/(1.0-R_frontal)

# ── 3. Test de consistencia: integrar EQE x AM1.5G ─────────────────
EQE_bins=np.interp(lam,lam_eqe,EQE)
Jsc_recon=q*(EQE_bins*Phi_bin).sum()*1e-4*1e3
err=(Jsc_recon-Jsc_broad*1e3)/(Jsc_broad*1e3)*100
print("\n"+"="*58)
...
print("\nGrafica guardada: etapa4_eqe.png")